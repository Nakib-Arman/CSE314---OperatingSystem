...
void logbook_entry(Operative* leader)
{
    usleep(leader->getPhase2Time() * SLEEP_MULTIPLIER);
    pthread_mutex_lock(&output_lock);
    cout<<"Unit "<<leader->getID()<<" has completed intelligence distribution at time "<<to_string(get_time())<<endl;
