...
void* document_recreation(void *arg)
{
    ...
    pthread_mutex_unlock(&output_lock);
    int operative_unit = ceil(operative->getID()/static_cast<float> (M));
    units[operative_unit - 1]->incPhase1Done();
    if(units[operative_unit - 1]->getPhase1Done() == M) {
        pthread_mutex_lock(&outpu)
    }
