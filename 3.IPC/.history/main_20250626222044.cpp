...
#define MAX_PHASE1_TIME 10
#define MAX_PHASE2_TIME 10
#define SLEEP_MULTIPLIER 1000

pthread_mutex_t stations[4];
...
public:
    Operative (int id) {
        this->id = id;
        phase1_time = (get_random_number() % MAX_PHASE1_TIME) * 1000;
...
void initialize() {
    for (int i=0;i<4;i++){
        ...
        pthread_mutex_init(&stations_lock[i]
    }
    for (int i=0;i<N;i++){
